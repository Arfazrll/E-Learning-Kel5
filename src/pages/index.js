      ...
      >
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {[
              { icon: <FiBook />, number: "10,000+", label: "Soal Latihan" },
              { icon: <FiCheckSquare />, number: "85%", label: "Tingkat Kelulusan" },
              { icon: <FiAward />, number: "50+", label: "Pengajar Expert" },
              { icon: <FiBriefcase />, number: "12,000+", label: "Siswa Aktif" }
            ].map((stat, index) => (
              <motion.div
                key={index}
                variants={{
              ...
              >
                <Card hover>
                  <div className="relative h-48 rounded-t-lg overflow-hidden -mx-5 -mt-5">
                    {/* Perubahan dari <image> menjadi <img> */}
                    <Image
                        src={course.image}
                        alt={course.title}
                        fill={true}
